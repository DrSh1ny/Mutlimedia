...
class Level {
  ...
  run() {
    ...
    function render(time) {
      ...
      if (gamestate == "end") {//evaluate ending conditions
        ...
        self.shotsHandler(char, assets, shooters);//projectils
        self.bulletHandler(char, bullets, assets, shooters, sounds);//projectils
        char.move(char, timePassed, ctx); //character movement

        camera.updateAnim(imagens, char, assets, assetsAnimated, shooters, bullets, mapa, ctx, self.background);//rendering of everything
        camera.drawHUD(ctx, char, imagens);//UI - pause and lives symbols
  ...
  clearLevel(canvas, assets, assetsAnimated, shooters, bullets, bulletFiredHandler, keyUpLevelHandler, sounds, imagens, id, elementos, levelSound, volumeInicial) {
    ...
    Object.keys(sounds).forEach(function (key, index) {//reset sound
